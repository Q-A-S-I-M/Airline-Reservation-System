...
public class AdminServiceImpl implements AdminService {
    ...
    private List<OccupancyRate> getOccupancyRatePerAirline() {
        String sql = "SELECT a.name AS airline, ROUND(AVG(f.booked_seats * 100.0 / f.total_seats), 2) AS occupancyRate FROM airlines a LEFT JOIN flights f ON a.id = f.airline_id WHERE f.status NOT IN ('Scheduled', 'Delayed', 'Cancelled') GROUP BY a.name";
